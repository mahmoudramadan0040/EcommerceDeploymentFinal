...
class ApiProductController extends Controller
{
        ...
        if (!empty($categories)) {
            ...
            foreach ($categories as $category) {
                $data[] = [
                    ...
                    'category_image' => env('AWS_URL') . '/' . $category->cat_image,
                ];
            }
    ...
    public function getProductItemsByProductId($product_id)
    {
        $productItems = ProductDetail::where('product_id', $product_id)->get();

        if (empty($productItems)) {
            $response =  $this->getResponse(0, 'noItems');
        } else {

            $response =  $this->getResponse(1, '', $productItems);
        }
        return response($response)->header('Content-Type', 'application/json');
    }
    ...
    public function getProductItemDetailsByItemId($item_id)
    {
        ...
        // $response =  $this->getResponse(1, '', $product_id);
        if (empty($productItemDetails)) {

            $response =  $this->getResponse(0, 'noItemDetails');
        } else {
            $product_id = $productItemDetails->product_id;
            $product = Product::where('id', $product_id)->first();
            $product_desc = $product->product_disc;
            $productItemDetails->product_desc = $product_desc;
